                            ...
                            const cv::Point &center, double angle)
{
    ...
    static const int lineKind = cv::LINE_8;              // the default
                                ...
                                const cv::Point &center, int radius)
{
    ...
    cv::circle(image, center, radius, colorRed, thickness, lineKind);
}
...
void drawWhiteRookFilledPolygon(cv::Mat &image)
{
    ...
    cv::fillPoly(image, ppt, npt, nContours, colorWhite, lineKind);
}
...
static void drawYellowRectangle(cv::Mat &image)
{
    ...
    static const int thickness = cv::FILLED;
    static const int lineKind = cv::LINE_8;
    cv::rectangle(image, vertex1, vertex2, colorYellow, thickness, lineKind);
}
                          ...
                          const cv::Point &end)
{
    ...
    static const int thickness = 2;
    static const int lineKind = cv::LINE_8;
    cv::line(image, begin, end, colorGreen, thickness, lineKind);
}
